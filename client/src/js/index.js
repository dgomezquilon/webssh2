...
let term = new Terminal({fontSize: '13', fontFamily: '\'Cousine\', Menlo, courier-new, courier, monospace'})
// DOM properties
let status = document.getElementById('status')
let header = document.getElementById('header')
let dropupContent = document.getElementById('dropupContent')
...
if (document.location.pathname) {
  ...
  let resource = base.substring(1) + 'socket.io'
...
function downloadLog () { // eslint-disable-line
  if (loggedData === true) {
    ...
    let blob = new Blob([sessionLog.replace(/[\u001b\u009b][[\]()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><;]/g, '')], {
      type: 'text/plain'
    })
