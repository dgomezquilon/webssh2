...
module.exports = function socket (socket) {
  ...
  socket.request.session.username = 'ubuntu'
  if (socket.request.session.username && socket.request.session.ssh) {
    // console.log('hostkeys: ' + hostkeys[0].[0])
    conn.connect({
      host: socket.request.session.ssh.host,
      port: socket.request.session.ssh.port,
      username: socket.request.session.ssh.username,
      passphrase: socket.request.session.ssh.passphrase,
      privateKey: socket.request.session.ssh.privateKey,
      tryKeyboard: true,
